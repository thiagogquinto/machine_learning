...
def imprimir_estatisticas(resultados):
    ...
    print(
        "Resultados: %.2f +- %.2f, min: %.2f, max: %.2f" % (media, desvio, mini, maxi)
    )
...
def rejeitar_hip_nula(amostra1, amostra2, alpha=0.05):
    ...
    _, pvalor = ttest_ind_from_stats(
        media_amostral1,
        desvio_padrao_amostral1,
        len(amostra1),
        media_amostral2,
...
def print_t_tests(resultados, cols=None, alpha=0.05):
    ...
    for t in sorted(resultados):
        ...
        for t2 in cols:
            ...
            print(
                ("%.02f%s" % (p, (" (*%c)" % dif) if d else "")).center(largura), end=""
            )
        print()


def selecionar_melhor_modelo(classificador, X_treino, X_val, y_treino, y_val, n_jobs=4, cv_folds=None, params={}):
    ...
    if cv_folds is not None:
        ...
        clf = GridSearchCV(
            classificador(), params, cv=cv_folds, n_jobs=n_jobs, scoring=score_fn
        )
        ...
        f1s_val = Parallel(n_jobs=n_jobs)(delayed(treinar_ad)(X_treino, X_val, y_treino, y_val, p) for p in param_grid)
...
def do_cv(classificador, X, y, cv_splits, param_cv_folds=None, n_jobs=8, scale=False, imputer=None, dim_red=None, params={},):
    ...
    for treino_idx, teste_idx in skf.split(X, y):
        ...
        ad, melhor_comb, _ = selecionar_melhor_modelo(classificador, X_treino, X_val, y_treino, y_val, n_jobs=n_jobs, cv_folds=param_cv_folds, params=params,)
        pred = ad.predict(X_teste)

        if len(set(y_treino)) > 2 and classificador == "knn" or classificador == "rf":
